...
for pollutant in pollutants:
    ...
    for combs in polcombs:
        if any(pollutant in s for s in combs):
            ...
            os.makedirs(outPath+'/Calibration/'+str(deviceId)+'/trainAndTest', exist_ok=True)
            dataTest.to_csv(outPath+'/Calibration/'+str(deviceId)+'/trainAndTest/test_'+
                     '-'.join(combi) +'.csv', index=False)
            dataTrain.to_csv(outPath+'/Calibration/'+str(deviceId)+'/trainAndTest/train_'+
                     '-'.join(combi) +'.csv', index=False)
